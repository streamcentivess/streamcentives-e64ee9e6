-- Phase 5: Advanced Analytics & Monetization Tables (Fixed)
...
CREATE INDEX IF NOT EXISTS idx_creator_analytics_creator_date ON public.creator_analytics(creator_id, date DESC);
CREATE INDEX IF NOT EXISTS idx_user_engagement_user_date ON public.user_engagement_metrics(user_id, date DESC);

-- Create function for analytics calculations (only if it doesn't exist)
